...
github-fetch cli --language Object-C --created 20220101 --sort stars --order desc --perPage 100 --page 1 --out line|console`,
...
func init() {
	...
	cliCmd.Flags().StringVar(&query.Language, "language", "", "需要制定编程语言的选项")
	cliCmd.Flags().StringVar(&query.Created, "created", "-1", "指定日期，默认不指定时间")
	cliCmd.Flags().StringVar(&query.Sort, "sort", "s", "数据的类别排序方式，默认 stars")
	cliCmd.Flags().StringVar(&query.Order, "order", "o", "数据排序，默认 desc")
	...
	cliCmd.MarkFlagRequired("language")
}
